...
Server-side upload — \`multer\` is included for buffering uploads in memory (no temp
files on disk — serverless friendly) when the upload must go through your API:
...
// in your service — buffer streams straight to Cloudinary
...
// persist result.public_id (and result.secure_url) on your document
\`\`\`
`;
  }
...
Backend API — MongoDB + Express 5 + Node (ESM), generated with \`men-backend\`.
